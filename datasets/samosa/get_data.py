"""Implements dataloaders for the SAMOSA dataset.

Here, the data is assumed to be in a folder titled "samosa".
"""
import numpy as np
...
def get_dataloader(data_dir, batch_size=40, num_workers=8, 
                   train_shuffle=True, 
                   flatten_imu=False,
                   unflatten_audio=True, 
                   unsqueeze_channel=True, 
                   normalize_imu=True, 
                   normalize_audio=True):
    """Get dataloaders for SAMOSA.
    ...
    trains = [np.load(data_dir+"/imu/train_data.npy"), np.load(data_dir +
                                                                 "/audio/train_data.npy"), np.load(data_dir+"/activity/train_data.npy")]
    tests = [np.load(data_dir+"/imu/test_data.npy"), np.load(data_dir +
                                                               "/audio/test_data.npy"), np.load(data_dir+"/activity/test_data.npy")]
